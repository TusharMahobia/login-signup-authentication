...
export default function Login() {
  ...
  function check(){
    ...
    const fetchApi = async()=>{
        ...
        const response = await fetch("/login",{method:"POST",credentials:"include",headers:{"Content-type": "application/json"},body: JSON.stringify(a)});
        const resJson = await response.json();
        console.log(resJson);
    ...
    <div>
      <div className="wrapper fadeInDown">
        <div id="formContent">
          ...
          {/* <!-- Login Form --> */}
          <input
            type="text"id="login"className="fadeIn second"name="login"placeholder="Username"onChange={(e) => {setusername(e.target.value)}}/><br></br><br></br>
          ...
          <input type="button" className="fadeIn fourth bg-success" value="Log In" onClick={check} /><br></br><br></br>
          {/* <!-- Remind Passowrd --> */}
          <div id="formFooter">
            <Link className="underlineHover" to="/signup">Signup</Link>
          </div>
        </div>
        {result ? (<div><h1>{result}{res}</h1></div>) : (<div><h1></h1></div>)}
      </div>
    </div>
  );
}
